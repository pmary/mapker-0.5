...
Meteor.methods({
	 ...
	 */
	invitePlaceStaffMembers: function (usersArray, placeId) {
		check(usersArray, Array);
		check(placeId, String);

		// Check if the user has the admin rigths over the place
		var testUserRights = Places.findOne({ $and: [ {_id: placeId}, {administrators: Meteor.userId()} ]});
		if (! testUserRights) return false;
			...
			Meteor.defer(function() {
				...
				for (var i = 0; i < usersArray.length; i++) {
					...
					if (usersArray[i].fullname) {
							// Get the user email
							var user = Meteor.users.findOne({_id: usersArray[i].id});
							users.push( user );

							// Check if the user was already a member of the place and update him
							var alreadyMember = Places.findOne({_id: placeId, members: {$elemMatch: { id: user._id }} }, { $addToSet: { 'grades.$.staff': true } });
							if (alreadyMember) {
								console.log('was alreadyMember');
								// Update the member by setting the 'staff' to true
								Places.update({_id: placeId, members: {$elemMatch: { id: user._id }} }, { $set: { 'members.$.staff': true } });
							}
							else {
									// Add the user as member and set him staff to true
									Places.update({_id: placeId}, { $addToSet: { members: {id: user._id, staff: true} } });
							}
					}
					return false;
				...
				for (var i = 0; i < users.length; i++) {
					...
					var subject = place.name + ' has indicated that you are a member of his staff';
					...
					if (users[i].profile.address.countryCode == 'FR') {
						...
						subject = place.name + ' a indiqué que vous faites parti de son équipe';
					}

					if (users[i].profile.avatar)
						userAvatar = users[i].profile.avatar.url;
					...
					HTTP.call("POST", "https://mandrillapp.com/api/1.0/messages/send-template.json",
		      	{
			        		...
			        		"from_name": "Mapker",
						      "to": [
						        { "email": /*users[i].emails[0].address*/'pierre.mary10@gmail.com', "name": users[i].profile.firstname, "type": "to" }
						      ],
						      ...
						      "global_merge_vars": [
										...
										{ "name": "USERNAME", "content": users[i].profile.firstname },
										{ "name": "PLACEAVATAR", "content": placeAvatar },
										{ "name": "PLACELINK", "content": 'http://mapker.co/places/' + place.name + '/about' },
