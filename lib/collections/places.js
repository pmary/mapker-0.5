...
Meteor.methods({
	 ...
	 */
	inviteStaffMembers: function (users, placeId) {
		check(users, Array);
		check(placeId, String);
		...
		if (Meteor.isServer) {
			Meteor.defer(function() {
				...
				for (var i = 0; i < users.length; i++) {
					...
					if (users[i].fullname) {
							// Get the user email
							var user = Meteor.users.findOne({_id: users[i].id});
							users.push( user );
				...
				for (var i = 0; i < users.length; i++) {
					...
					var userAvatar = '//localhost:3000/images/avatar-user-default.png';
					var placeAvatar = '//localhost:3000/images/avatar-place-default.png';
					...
					HTTP.call("POST", "https://mandrillapp.com/api/1.0/messages/send-template.json",
		      	{
			        		...
			        		"from_name": "Mapker",
						      ...
						      "global_merge_vars": [
										...
										{ "name": "USERLINK", "content": '//mapker.co/user/' + users[i]._id + '/bio' },
										{ "name": "USERNAME", "content": users[i].profile.firstname },
										{ "name": "PLACEAVATAR", "content": placeAvatar },
										{ "name": "PLACELINK", "content": '//mapker.co/places/' + place + '/about' },
