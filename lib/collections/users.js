Meteor.methods({
	...
	userUpdateIdentity: function(userIdentity) {
		...
		var userId = Meteor.users.update({_id: user._id}, { $set: {
			...
			'profile.countryCode': userIdentity.countryCode,
			'profile.zipcode': userIdentity.zipcode
		} });
		return {_id: userId};
	}
});
...
validateUserAddSkill = function(skill, skills) {
	...
	if (skillError) {
		errors.skill = skillError;
	}

	if (skills && skills.length) {
		var duplicateSkillError = isDuplicateSkillValidation(skill, skills);
		if (duplicateSkillError) {
			errors.skill = duplicateSkillError;
		}
	};
