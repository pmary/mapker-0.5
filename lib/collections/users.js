...
Meteor.methods({
	 ...
	 */
	userSendConnectionRequest: function (userId) {
		check(Meteor.userId(), String);
		check(userId, String);

		var user = Meteor.user();

		// Update the 'pending' field of the current user with the given id
		var pendingResult = Meteor.users.update({_id: user._id}, { $addToSet: {'profile.network.users.pending': userId} });

		// Update the 'requests' field of the newsly followed user who revieve the request
		var requetsResult = Meteor.users.update({_id: userId}, { $addToSet: {'profile.network.users.requests': user._id} });

		// Push a follow notification to user who revieve the request
