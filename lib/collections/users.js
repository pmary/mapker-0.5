...
Meteor.methods({
	...
	userUnConnect: function (userId) {
		...
		var disconnectResult = Meteor.users.update({_id: user._id}, {$pull: {'profile.network.users.connected': userId} });
		var disconnectResult = Meteor.users.update({_id: userId}, {$pull: {'profile.network.users.connected': user._id} });
