...
Template.modalSuggestPlace.helpers({
...
Template.modalSuggestPlace.rendered = function () {
	$('.modal-suggest-place [data-toggle="popover"]').popover();
		...
		if (! selectizeCountry) {
			...
			if (countries.length == 245) {
				...
				setTimeout(function () {
					...
					var $selectContry = $('.modal-suggest-place select#select-country').selectize({
						maxItems: 1
					});
...
Template.modalSuggestPlace.onDestroyed(function () {
...
Template.modalSuggestPlace.events({
