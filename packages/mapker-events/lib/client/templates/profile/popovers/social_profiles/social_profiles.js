var editEventSocialProfilesClickListener = function () {
  $('.update-social-profile-popover').css('display', 'none');
};

var editEventLocationClickListener = function () {
...
Template.eventUpdateSocialProfiles.helpers({
  staticMapUrl: function() {
		return Session.get('staticMapUrl');
	},
	errorMessage: function(field) {
    if (Session.get('eventUpdateSocialProfilesErrors')) {
		  return Session.get('eventUpdateSocialProfilesErrors')[field];
    }
	},
	errorClass: function (field) {
    if (Session.get('eventUpdateSocialProfilesErrors')) {
		  return !!Session.get('eventUpdateSocialProfilesErrors')[field] ? 'has-error' : '';
    }
	}
});

Template.eventUpdateSocialProfiles.onDestroyed(function () {
  // Cancel the click listener
  document.body.removeEventListener('click', editEventSocialProfilesClickListener, false);
});

Template.eventUpdateSocialProfiles.events({
  /**
    ...
    editEventLocationClickListener();
    document.body.removeEventListener('click', editEventLocationClickListener, false);
    ...
    document.body.addEventListener('click', editEventSocialProfilesClickListener, false);
	},
  /**
  ...
  'click': function (e, t) {
    ...
    // Switch the button to the load state
		...
		Session.set('eventUpdateSocialProfilesErrors', errors);
  ...
  'click .open-update-place-location-popover': function (e, t) {
    ...
    editEventSocialProfilesClickListener();
    document.body.removeEventListener('click', editEventSocialProfilesClickListener, false);
    ...
    // Set a click event listener on the body to close the popover when the user click outstide
    document.body.addEventListener('click', editEventLocationClickListener, false);
  },
  /**
